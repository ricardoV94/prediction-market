...
from discord_bot.market_description import (
    create_market_embed,
    market_sentiment,
    update_market_top_post,
)
...
if __name__ == "__main__":
    # Parse cli arguments
    # Parse cli arguments
    parser = argparse.ArgumentParser(description="Discord Prediction Market bot")
    parser.add_argument(
        "--update-threads",
        action="store_true",
        help="Update existing market threads and create missing ones on bot init.",
    )
                ...
                if market_id in exchange_markets:
                    ...
                    if UPDATE_THREADS:
                        await update_market_top_post(
                            thread, exchange_markets[market_id]
                        )
                else:
                    LOGGER.warning(
        ...
        for market_id, market in sorted(EXCHANGE.markets.items()):
            if market_id not in markets_with_threads:
                ...
                thread_with_message = await forum_channel.create_thread(
                    name=f"{market.question} (#{market.id})",
